...
add_instruction(&spicy_brownies_with_pomegranate_syrup_brownies, "Pour into a greased 8X8 baking dish. To improve the flavor of the brownies, place the unbaked batter in refrigerator <u>overnight (or a few hours)</u>. If you don't want to wait, place baking dish in oven and bake for <u>~20 minutes</u>. Add <u>2 minutes</u> if using vegan butter. To check if it's ready, insert a toothpick into the center, if it comes out with a few moist crumbs attached to it, it's ready. Remove from oven and let cool. <i>Cut in 24 small squares</i>.");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &canola_oil, "90 ml");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &dark_chocolate, "250 g");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &flax_seeds, "45 g");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &water, "135 ml");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &brown_sugar, "75 g");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &granulated_sugar, "100 g");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &chili_pepper_flakes, "30 g");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &cayenne_pepper_powder, "2.5 g");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &salt, "1.25 g");
add_serving(&spicy_brownies_with_pomegranate_syrup_brownies, &all_purpose_flour, "140 g");
...
Recipe *recipes[] = {&anise_bread_with_sweet_pear_sauce, &arame_soba, &balsamic_banana_ice_cream, &black_sesame_brittle, &black_sesame_rice_pancakes, &black_sesame_syrup, &breaded_veggie_fingers, &carrot_kinpira_onigirazu, &cheese, &cheese_and_spinach_ravioli, &basic_black_bread, &basic_toothpaste, &beer_bread, &borscht_with_tofu_sour_cream, &breadfruit_gnocchi, &breadfruit_pasta, &buckwheat_tea, &chickpea_salad_sandwich, &chocolate_chip_cookies, &coffee_jelly, &corn_dumplings, &crackers, &dark_gyoza, &fresh_pesto_pasta, &halloween_pumpkin_cookies, &veganaise, &hop_ice_cream, &houjicha_overnight_oatmeal, &lentils_with_roasted_beet_sauce, &mason_jar_bread_pudding, &mushroom_zucchini_pasta, &mustard_from_seed, &no_knead_bread, &okonomiyaki, &pandanus_fruit_bread, &pan_fried_breadfruit, &papaya_bruschetta_topping, &pate_chinois, &persimmon_curry, &potato_gnocchi, &quick_sunflower_seed_parmesan, &raisin_beet_bread, &roasted_carrots_with_beluga_lentils, &roasted_pumpkin_seeds, &scrambled_chickpeas, &spicy_brownies_with_pomegranate_syrup, &sunflower_heirloom_carrot_pasta, &sweet_mock_eel_nigiri, &teriyaki_veggie_patties, &turmeric_cookies, &tzaziki, &salted_caramel_carob_chip_cookies, &seitan, &shichimi_togarashi_crackers, &spicy_stirfry_chickpeas, &spinach_oatmeal_cookies, &spinach_pajeon, &stovetop_popcorn, &sweet_and_sour_lentils, &uzumaki_hummus_bites, &vegemite_caramel, &veggie_pate, &wakame_bites, &wasabi_swirl_chocolate_cookies };